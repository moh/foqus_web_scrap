...
class GeneralSpider(scrapy.Spider):
    ...
    def start_requests(self):
        home_pages = dict() # this will contain all home pages and categories of a url_base with the categories
        home_product_pages = list() # this will contains lists that contain  lines for first appearance of a url_base
        with open("sites.csv", "r") as f:
            reader = csv.reader(f, delimiter = ",")
            for line in reader:
                url_base = self.getUrlBase(line[0])
                if url_base in home_pages:
                    home_pages[url_base].append([line[0]] + line[2:]) # append a list of home url and categories
                else: # first home page of this url_base
                    home_pages[url_base] = list()
                    home_product_pages.append(line)
                    self.url_home_products[url_base] = [line[0], line[1]]

        print("home_pages : ", home_pages)
        print("home_product_pages : ", home_product_pages)

        for data in home_product_pages:
            url = data[0] # url of home page
            url_base = self.getUrlBase(url)
            yield Request(url, callback = self.getHomeClasses, meta = {"categories" : data[2:], "other_pages" : home_pages[url_base]}) # pass the list of other home pages 
    ...
    def getHomeClasses(self, response):
        ...
        yield Request(self.url_home_products[self.getUrlBase(response.url)][1],
                      callback = self.getProductClasses, meta = response.meta)
    ...
    def getProductClasses(self, response):
        ...
        final_classes = set()
        other_pages = response.meta.get("other_pages") # get other home pages of same domain
        ...
        # call the home page
        yield Request(self.url_home_products[self.getUrlBase(response.url)][0],
                      meta = {"categories" : response.meta.get("categories"), "home":True }, dont_filter = True)

        for data in other_pages:
            yield Request(data[0], meta = {"categories" : data[1:], "home":True}) # request of other home pages with the categories
    ...
    def parse(self, response):
        ...
        if (not(infos["is_home"]) and not(infos["is_product"])):
            ...
            data = self.getProductInfo(response, infos["product_shared"])
            data["categories"] = categories # add category to data
            yield data
        ...
        # if it is not home page, then stop crawling
        ...
        #self.order -= 1
        for link in links:
            yield Request(link, meta = {"categories" : categories})#, priority = self.order)
    ...
    def spider_quit(self, spider):
        ...
        data = {"url_home_products":self.url_home_products, "url_classes" : url_classes,
                ...
                "titleImg_method_classes": self.titleImg_method_classes}
        print("\n ^^^^^^^^^^^^^^ \n STORING TO FILE \n ^^^^^^^^^^^^^^ \n")
        print("data = ", data)
